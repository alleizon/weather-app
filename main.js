...
const DOM = (() => {
  ...
  const currentIcon = (weather) => {
    ...
    const prefix = "wi-";
    const { id } = weather;
    let { icon } = _assets_icons_json__WEBPACK_IMPORTED_MODULE_0__[id];
  ...
  const updateCurrent = (obj, unit) => {
    ...
    const { timezone } = obj;
    const dateObj = _helpers__WEBPACK_IMPORTED_MODULE_1__["default"].getLocalDate(timezone);
    const formattedDate = _helpers__WEBPACK_IMPORTED_MODULE_1__["default"].formatDate(dateObj);
  ...
  const updateForecast = (data, unit) => {
    ...
    data.forEach((day) => {
      ...
      document
        ...
        .addEventListener("click", _helpers__WEBPACK_IMPORTED_MODULE_1__["default"].renderTimestamps);
...
const Helpers = (() => {
  const formatDateForecast = (date) => (0,date_fns__WEBPACK_IMPORTED_MODULE_0__["default"])(date, "EEEE h aaa");

  const formatDate = (date) => (0,date_fns__WEBPACK_IMPORTED_MODULE_0__["default"])(date, "EEEE dd LLL HH:mm").split(" ");
...
const Process = (() => {
  ...
  const forecastWeather = (list, timezoneOffsetMS) => {
    ...
    list.forEach((timestamp, index) => {
      ...
      const formattedDate = _helpers__WEBPACK_IMPORTED_MODULE_0__["default"].formatDateForecast(date).split(" ");
      const [day, time] = [
        formattedDate[0],
...
function request(newCity = city) {
  _requestAPI__WEBPACK_IMPORTED_MODULE_0__["default"].call(newCity, unit)
    .then(([currentResult, forecastResult]) => {
      const obj = _processors__WEBPACK_IMPORTED_MODULE_1__["default"].currentWeather(currentResult);
      _DOM__WEBPACK_IMPORTED_MODULE_2__["default"].updateCurrent(obj, unit);

      const { list } = forecastResult;
      const timezoneMS = forecastResult.city.timezone * 1000;
      const forecastInfo = _processors__WEBPACK_IMPORTED_MODULE_1__["default"].forecastWeather(list, timezoneMS);

      _DOM__WEBPACK_IMPORTED_MODULE_2__["default"].updateForecast(forecastInfo, unit);
    })
    .catch((err) => {
      if (err.cause === "http-error") {
        _DOM__WEBPACK_IMPORTED_MODULE_2__["default"].displayError("invalid city");
      } else _DOM__WEBPACK_IMPORTED_MODULE_2__["default"].displayError("network error");
    });
}
...
backBtn.addEventListener("click", _helpers__WEBPACK_IMPORTED_MODULE_3__["default"].removeTimestamps);
...
celsiusBtn.addEventListener("click", () => {
  ...
  unit = newUnit;
  _DOM__WEBPACK_IMPORTED_MODULE_2__["default"].swapUnits(unit);
});
